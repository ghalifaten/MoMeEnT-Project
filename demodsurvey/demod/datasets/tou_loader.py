...
class LoaderTOU(DatasetLoader):
    ...
    def load_activity_probability_profiles(
        ...
        *Array Shapes: DIM0: Time, DIM1:Active_Occupants
        ...
        file_name = os.path.join(
            self.activity_type,
            approach + '_activity_profiles_' + subgroup_str)
    ...
    def load_avg_activity_prob(
        ...
        try:
            act_probs = dict(np.load(file_path + '.npz'))
        except FileNotFoundError as err:
            self._warn_could_not_load_parsed(err, file_name)
            act_probs = self._parse_avg_activity_prob(
                subgroup
            )
            np.savez(file_path, **act_probs)
        return act_probs
