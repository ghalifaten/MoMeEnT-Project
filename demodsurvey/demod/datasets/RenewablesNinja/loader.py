...
class NinjaRenewablesClimate(ClimateLoader):
    ...
    def _parse_historical_climate_data(self) -> Dict[str, np.ndarray]:
        ...
        out_dict['irradiance_surface'] = np.array(df['irradiance_surface'])
        out_dict['irradiance_toa'] = np.array(df['irradiance_toa'])
        out_dict['irradiance'] = np.array(df['irradiance_surface'])
