    ...
    * :py:meth:`~demod.datasets.DESTATIS.loader.Destatis`
    ...
    def __init__(self, /, version: str = "v0.1", **kwargs) -> Any:
        ...
        if version == "v0.1":
            ...
            self.activity_data = GTOU("DemodActivities_1")
        else:
        ...
        # initialize inputexcel loader
        self.input_excel = InputFileLoader(raw_file_path=self.raw_file_path)
    ...
    def load_activity_probability_profiles(
        self, subgroup: Subgroup, approach: str = 'CREST',
    ) -> Dict[str, np.ndarray]:
    ...
    def load_avg_activity_prob(self, subgroup: Subgroup):
        return self.activity_data.load_avg_activity_prob(subgroup)
    ...
    def _parse_appliance_dict(self) -> AppliancesDict:
        if self.version in ["v0.1", "vBottaccioli", "v1.1"]:
    ...
    def _parse_yearly_target_switchons(
        ...
        return self.input_excel.load_yearly_target_switchons(subgroup)
