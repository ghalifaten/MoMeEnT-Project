...
class SubgroupsIndividualsActivitySimulator(
    ...
    def get_individual_doing_activity(self, activity):
        """Return the activity of each person"""
        if isinstance(activity, list):
            return [self.get_n_doing_activity(a) for a in activity]
        # merges the results of all the subsimulators
        persons_states = np.concatenate(
    ...
    def get_individual_activity_states(self) -> Dict[str, np.ndarray]:
        """Return a dictionary containing the state of each person."""
        dict_states = {
            lab: self.get_individual_doing_activity(lab) 
            for lab in self.activity_labels
        }
        
        # convert dictionary into unique array
        # TODO: use index of acticity instead of activity names
        states = np.zeros_like(dict_states[self.activity_labels[0]], dtype='S20')
        for act, bool_state in dict_states.items():
            states[bool_state] = [act]
    ...
    def get_occupancy(self, states=None) -> np.array:
        ...
        states_away = np.zeros_like(self.n_residents)  
        for k,v in states.items():
            if 'away' in k or 'transport' in k:
                states_away += v
        return self.n_residents - states_away
