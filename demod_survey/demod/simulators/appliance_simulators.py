...
class AppliancesSimulator(TimeAwareSimulator):
    ...
    The attributes below show more in depth which information are
    stored.
    ...
    def sample_available_appliances(
        self,
        equipped_sampling_algo: str = "basic",
        equipped_set_defined: Union[dict, np.ndarray] = None,
        **kwargs
    ) -> np.ndarray:
        ...
        Args:
            equipped_sampling_algo: The method used for sampling.
                ...
                :set_defined:
                    ...
                    :py:attr:`appliance_set`. If a dictionary is passed, 
                    the set of appliances is the same across all households, 
                    while different sets can be specified if a np.array of size 
                    (N households x M appliances) is passed
                :all:
        ...
        if equipped_sampling_algo == "basic":
            ...
            # Ex. you own a dryer only if you have a washing machine
    ...
    def _appliance_set(self, equipped_set_defined):
        # same set of appliances for all households
        if isinstance(equipped_set_defined,dict):
        ...
        # specific set of appliances for each household
        elif (
                isinstance(equipped_set_defined, np.ndarray)
                and equipped_set_defined.dtype == bool
                ):
            if equipped_set_defined.shape == (self.n_households,
                                       len(self.appliances['name'])):
                return equipped_set_defined
            else:
                raise ValueError(
                    "Please pass an array with shape "
                    f"({self.n_households}, {len(self.appliances['name'])}) "
                    f"and not {equipped_set_defined.shape}"
                    )
        else:
            raise ValueError(
                "Please pass a dictionary or an array (dtype=bool) with "
                f"shape ({self.n_households}, {len(self.appliances['name'])}) "
                ) 
    ...
    def sample_real_load_profiles(
        ...
        Uses
        :py:meth:`~demod.datasets.base_loader.ApplianceLoader.load_real_profiles_dict`
        to get real load profiles for the appliances.
    ...
    def switch_on(self, indexes_household, indexes_appliance):
        ...
        Args:
            indexes_household: list or array of the index of the household.
            indexes_appliance: list or array of the index of the appliances, 
                                matching with the households.
        """
...
class ActivityApplianceSimulator(AppliancesSimulator):
    ...
    :py:class:`.ProbabilisticActivityAppliancesSimulator`
    ...
    def _remove_non_compatible_appliances(
        ...
        if 'probabilistic' in appliance_dict:
            # Needs probabilistic key to remove these appliances
    ...
    def step(self, activities_dict: ActivitiesDict,
             updated_appliance_set: np.ndarray = None) -> None:
        ...
        if updated_appliance_set is not None:
            # Update the available appliances using external data
            self.available_appliances = self._appliance_set(updated_appliance_set)
        ...
        for act, n_performing in activities_dict.items():
            ...
            # Get the ones who performed the activity during last step
            previous_performing = self.previous_act_dict[act]
            # Gets the ones that have changed activity
            mask_change = previous_performing != n_performing
            
            #TODO: include probability of turning ON only 1 or 2 appliances
                
            # Switch off
            mask_hh_act_stops = mask_change & (n_performing == 0)
            mask_app_stop = (
                mask_apps_this_act & self.appliances['inactive_switch_off']
            ) # mask appliances that stop if the activity change
            ...
            if np.any(mask_hh_act_start):
                # turn off app if the household change activity and 
                # switch on depends on the activity
...
class ProbabilisticActivityAppliancesSimulator(AppliancesSimulator):
    """Appliance simulator based on residents activities.
    ...
    def __init__(
        self, n_households: int,
        initial_activities_dict: ActivitiesDict,
        data: DataInput = GermanDataHerus('v1.1'),
        subgroups_list: Subgroups = None,
        n_households_list: List[int] = None,
        **kwargs
    ):
    ...
    def sample_switch_on_probs(
        ...
        for i, subgroup in enumerate(self.subgroups_list):
            ...
            # The switchon targets for each appliance, TODO: check other targets
            ...
            for activity, act_prob in activity_prob.items():
                ...
                # This appliance can start every occurence of the activity
    ...
    def step(self, activities_dict: Dict[str, np.ndarray],
             updated_appliance_set: np.ndarray=None) -> None:
...
class AvailableOccupancyApplianceSimulator(AppliancesSimulator):
    ...
    *FOR FUTURE IMPLEMENTATION* Switchon probabilites should be compuuted
    ...
    def __init__(
        ...
        **kwargs
        ...
        # pass the dictionary to describe the energy service patterns
        if isinstance(usage_patterns,dict):
            self.usage_patterns = usage_patterns 
        else:
            raise ValueError("usage_patterns should be a dictionary")
    ...
    def initialize_starting_state(
        ...
        self.set_esp_time_pdf(self.usage_patterns['day_prob_profiles'])
        self.switch_on_prob_multiplier = self.get_switch_on_prob_multiplier()
    ...
    def _create_activities_labels(self):
        ...
        self.activities_labels = np.append(activities_labels, 'other')
    ...
    def set_activity_pdf(self):
        ...
        act_profiles = [
            self.data.load_activity_probability_profiles(subgroup,
                                                approach='avail_occupancy',
                                                )
            for subgroup in self.subgroups_list
        ]
        # DIM0: subgroup, Dict[DIM1:n_times, DIM2:active_occupancy
    ...
    def set_esp_time_pdf(self, esp_time_probs):
        """Set the energy service probability profiles for this simulator.
        """
        # TODO: Check if right format
        # TODO: when to start with it?
        if isinstance(esp_time_probs,dict):
            # TODO: verify that the keys use correct names of appliances
            # veryfy that each the data for each appliances have the right format
            # if esp_time_probs.shape == (self.n_households,24*6):
            print("n_houselholds = ", self.n_households)
            time_probs = np.asarray([
                esp_time_probs[app]
                if app in esp_time_probs.keys()
                else np.ones((self.n_households, 24))
                for app in self.appliances['name'] 
                ]
            )
            
            self.esp_time_pdf_iterator = itertools.cycle(
                np.moveaxis(time_probs, [-1, 0], [0, -1])
            )
            # DIM0: household, DIM1: appliances, DIM2: hours of the day
            
        else:
            raise ValueError((
                "Please pass for each appliance an array with shape ('{}','{}') " 
                "and not {}"
                ).format(self.n_households,
                         24,
                         esp_time_probs.shape))
    ...
    def get_switchon_probs(self, active_occupancy: np.ndarray) -> np.ndarray:
        ...
        # DIM0: subgroups, DIM1: activity, DIM2: active_occupancy/other
        ...
        # standard calculated on the excel
        out *= self.switch_on_prob_multiplier 
    ...
    def get_switch_on_prob_multiplier(self) -> np.ndarray:
        """Get the switchon probability multiplier for each appliance
        based on target number of cycles and average activity probability.
        In CREST this is pre-calculated in the excel sheet and average for 
        any household.
        Here it is calculated based on target number of cycles and 
        subgroup activity probabilities. 

        Returns:
            switchon_probs: array of size (n_households, n_appliances)
        """
        
        # set up the array for the switch on probability multiplier
        out = np.ones((self.n_households, self.appliances["number"]))
        
        # if target number of cycles per year is provided
        if self.usage_patterns:
            if "target_cycles" in self.usage_patterns.keys():
              
                mask_app = np.isin(
                    self.appliances['name'], 
                    list(self.usage_patterns['target_cycles'].keys())
                    )
                
                # provided target number cycles 
                switch_on_targets = np.asarray([
                    self.usage_patterns['target_cycles'][app]
                    for app in self.appliances['name'] 
                    if app in self.usage_patterns['target_cycles'].keys()
                    ]
                ).T
                
                # average probability per activity
                avg_probs = np.asarray([
                    [
                        # Ensures the activity are sorted as in
                        # self.activities_labels
                        self.data.load_avg_activity_prob(g)[act_lab]
                        if act_lab in self.data.load_avg_activity_prob(g).keys()
                        else 0
                        for act_lab in self.activities_labels
                    ]
                    for g in self.subgroups_list
                    ]
                )
                
                # indeces of the appliance-related activities
                act_index = np.asarray([
                    np.where(self.activities_labels == act)[0][0]
                    for app, act in zip(self.appliances['name'],
                                      self.appliances['related_activity'])
                    if app in self.usage_patterns['target_cycles'].keys()
                    ]
                )
                
                # index of the simulated activity category
                other_index = np.where(
                    self.activities_labels == 'other')[0][0]
                
                # available time
                avail_time = 365.25 * 24 * 60 * 60 / \
                    self.step_size.total_seconds() * \
                        avg_probs[self.hh_types[:,np.newaxis], other_index] - \
                            switch_on_targets * \
                                self.appliances['mean_duration'][mask_app]
                    
                # calculate
                # DIM0: households, DIM1: appliances
                switch_on_prob = np.zeros_like(out)
                switch_on_prob[:,mask_app] = switch_on_targets
                switch_on_prob[:,mask_app] /= avg_probs[
                    self.hh_types[:,np.newaxis],
                    act_index[np.newaxis,:]
                    ]
                switch_on_prob[:,mask_app] *= avg_probs[
                    self.hh_types[:,np.newaxis], other_index 
                    ]
                switch_on_prob[:,mask_app] /= avail_time
                
                out[:,mask_app] *= switch_on_prob[:,mask_app]
                out[:,~mask_app] *= self.appliances[
                    "switch_on_prob_crest"][~mask_app]
        else:
            # standard calculated on the excel
    ...
    def step(self, active_occupancy: np.ndarray) -> None:
        ...
        Args:
            avail_occupancy: array of available occupancy in each household.
        """
        # check active_occupancy
        active_occupancy_ = np.array(active_occupancy, dtype=int)
        assert (
            len(active_occupancy_) == self.n_households
        ), "active occupancy length must match the number of households"
        ...
        # # keeps track of the iterations to update the activities pdf
        if (self.current_time.minute % 60) == 0:
            # update the weighting energy service pdf every 60 min
            self.current_esp_time_pdf = next(self.esp_time_pdf_iterator)
            # DIM0: household, DIM1: hour of the day

        switchon_probs = self.get_switchon_probs(active_occupancy_) 
        
        # energy service pattern weighting factor 
        switchon_probs *= self.current_esp_time_pdf
