...
class GTOU(LoaderTOU, PopulationLoader):
    ...
    def _load_rawdata_info(self):
        ...
        corresponding_dict = {
            ...
            'DemodActivities_1': GTOU_label_to_Bottaccioli_act,
        }
        # Adds a state called 'away' when people are not at home
        add_away_state = {
            'DemodActivities_0': True,
            'Bottaccioli2018': True,
            'DemodActivities_1':False,
        }
        # Define a dictionary of activities when people are away
        away_dict = {
            'DemodActivities_0': False,
            'Bottaccioli2018': False,
            'DemodActivities_1': GTOU_label_to_away_binaryactivity,
        }
        # Whether to replace 'other' activities by secondary states when possible
        use_secondary_states = {
            'DemodActivities_0': True,
            'Bottaccioli2018': True,
            'DemodActivities_1':True,
        }
    ...
    def _parse_tpm(self, subgroup: Subgroup):
        if self.activity_type == '4_States':
        ...
        elif self.activity_type == 'DemodActivities_1':
            # Imports the raw data parser only here to save import time
            from .parser import (
                get_tpms_activity, GTOU_label_to_Bottaccioli_act,
                GTOU_label_to_away_binaryactivity
            ...
            return get_tpms_activity(
                ...
                add_away_state=False,
                away_dict=GTOU_label_to_away_binaryactivity,
                add_durations=False,
            )
        else:
    ...
    def _parse_activity_profiles(
        self, subgroup: Subgroup, approach: str
    ) -> Dict[str, np.ndarray]:
        from .parser import create_data_activity_profile
        return create_data_activity_profile(subgroup, approach=approach)
    ...
    def get_states(
        ...
        if self.add_away_state:
            ...
            away_states_replace_dict = {k: inv_new_states_dict.get(v, v) 
                                        for k, v in self.away_dict.items()}
    ...
    def _parse_daily_activity_starts(
        ...
        check_away_state = np.vectorize(lambda x: 'away' in x or 'travel' in x)    
        daily_starts['active_occupancy'] = np.bincount(
            ...
            np.bincount(  # Counts how many time each household start act
                transitions_dict['hh_day'][
                    ...
                    | check_away_state(transitions_dict['new_states'])
                ]
        )).astype(float) / len(states)  # Convert to probs
    ...
    def _parse_activity_probabilities(
        ...
        act_probs_away = np.zeros_like(activity_probabilities['sleeping'])
        for k,v in activity_probabilities.items():
            if 'away' in k or 'travel' in k:
                act_probs_away += v
    ...
    def _parse_avg_activity_prob(
        ...
        # Convert states to probabilities by household
        # map the states      
        avg_activity_prob = {
            s: (states == s)
            for s in np.unique(states)
        ...
        # map active_occupancy excluding sleeping, away and travel states
        active_states = [k for k in avg_activity_prob.keys() 
                           if 'away' not in k 
                           and 'travel' not in k 
                           and 'sleeping' != k
                           ]
        avg_activity_prob['active_occupancy'] = np.zeros_like(
            avg_activity_prob['sleeping'], dtype=bool
            )
        for s in active_states:
            avg_activity_prob['active_occupancy'] |= (states==s)
        
        # average time proportion when at least someone in the household
        # is engaged in each activity
        for k,v in avg_activity_prob.items():
            avg_activity_prob[k] = np.array([
                np.apply_along_axis(any, 0 , v[hh_id == hh])
                for hh in np.unique(hh_id)]
                ).sum(axis=0).mean() / len(np.unique(hh_id))
            
        return avg_activity_prob
